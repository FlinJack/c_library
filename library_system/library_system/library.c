...
//声明函数
void load_books();              //加载图书信息
void load_users();              //加载用户信息
void save_books();              //保存图书信息
void save_users();              //保存用户信息
void admin_system();            //管理员系统
void user_system();             //用户系统
void add_book();                //添加书籍
void view_books();              //查看书籍
void borrow_book();             //借阅书籍
void return_book();             //归还书籍
void delete_book();             //删除图书
void press_to_continue();       //新增的等待用户输入继续的函数
int get_valid_int_input();      //输入验证函数
void password_admin();          //管理员密码验证功能
void exit_project();            //退出程序函数
void view_borrowed_books();     //输出已借图书
void view_pointed_book();       //查找指定编号的图书
